...
from base import semeval17_main_test
...
class Word2VecRetrofit():
    ...
    def build_graph(self, vectors, alpha):
        ...
        self.inputs=[input1, input2, input_length1, input_length2, weight, learning_rate]
        ...
        def cosin_distance(a,b,axis=-1):
            normalize_a = tf.nn.l2_normalize(a,axis)
            normalize_b = tf.nn.l2_normalize(b,axis)
            d=1-tf.reduce_sum(tf.multiply(normalize_a,normalize_b),-1)
            return d
        self.synonym_loss = synonym_loss = tf.reduce_mean(cosin_distance(ave1, ave2)*castf(weight))
        ...
        offset = cosin_distance(tf.nn.embedding_lookup(W, input_), tf.nn.embedding_lookup(W_old, input_))*\
                 tf.nn.embedding_lookup(Alpha, input_)
        self.offset_loss = offset_loss =\
            5*tf.reduce_sum(castf(offset)*mask)/tf.cast(tf.reduce_sum(length_),config.dtype2)
    ...
    def run_eval(self,sess):
        ...
        #semeval_main_test(config.new_vec_path)
